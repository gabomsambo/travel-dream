...
export interface ProcessQueueResult {
  ...
  remaining: number;
  stoppedBecause: 'queue-empty' | 'deadline' | 'max-items';
  elapsedMs: number;
}
...
export async function processQueue(options: ProcessQueueOptions = {}): Promise<ProcessQueueResult> {
  ...
  const lane = async (): Promise<void> => {
    while (true) {
      ...
      const source = await claimNextQueuedSource(leaseId);
      if (!source) {
        reserved--;
        stopped ??= 'queue-empty';
        return;
...
function isTransientUpstreamError(error: unknown): boolean {
  ...
  const message = error instanceof Error ? error.message : String(error);
  return /\b(429|500|502|503|504)\b|overloaded|rate.?limit|quota|too many requests|^fetch failed|ECONNRESET|ETIMEDOUT|ENOTFOUND|socket hang up|network error/i.test(
    message
  );
}
...
async function runPipeline(source: Source, leaseId: string, signal: AbortSignal): Promise<number> {
  ...
  if (!extraction || !thumbnailUrl) {
    ...
    if (!extraction) {
      ...
      extraction = await withAbort(
        getGeminiExtractionService().extractFromImage(buffer, fileName),
        signal
      );
